  ...
  if(lastchat != owopbot.chat && !owopbot.chat.replace(/<(?:.|\n)*?>/gm, '').startsWith('[D]') && !owopbot.chat.replace(/<(?:.|\n)*?>/gm, '').startsWith('Server: You are now an admin. Do /help for a list of commands.') && !owopbot.chat.replace(/<(?:.|\n)*?>/gm, '').startsWith('Nickname') && !owopbot.chat.replace(/<(?:.|\n)*?>/gm, '').startsWith('(AO)') && !owopbot.chat.replace(/<(?:.|\n)*?>/gm, '').startsWith('<') && !owopbot.chat.replace(/<(?:.|\n)*?>/gm, '').startsWith('DEVWorlds'))
       {
		...
		function say(nessage) {
		  ...
		  DiscordClient.guilds.get("515657823045287965").channels.get("521378443019943969").send(nessage) //first isserver id second is channel id
...
DiscordClient.on("message", message => {
    ...
    if (message.channel.id != '521378443019943969') //channel id gateway discord
    return;
    var msg = message
...
function status1() {
  try{
  DiscordClient.user.setActivity("https://owoppa.netlify.com/", {type: "STREAMING"}); //your name of server 
  } catch(e) {
    console.log(e)
  }
...
function status2() {
  try{
  DiscordClient.user.setActivity("Merry Christmas", {type: "STREAMING"}); //funny message
  } catch(e) {
    console.log(e)
  }
